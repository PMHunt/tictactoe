...
* [`src/cljs/tictactoe/`](src/cljs/tictactoe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/tictactoe/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `tictactoe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/tictactoe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns tictactoe.example
  (:require [tictactoe.config :as config])
